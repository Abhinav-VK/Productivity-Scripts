  ...
  if (isFeatureEnabled('ilacAutoAttach') &&
    ...
    function ilacGetCaseStatus() {
      ...
      return null;
          ...
          function ilacGetCaseOwner() {
      // Method 1: Find using kat-table structure (same as status)
      const rows = document.querySelectorAll('kat-table-body kat-table-row');
      ...
      for (const row of rows) {
        ...
        if (cells.length >= 2) {
          ...
          if (label === 'Owner' || label === 'Owner:' || label === 'Case Owner' || label === 'Case Owner:') {
            return value;
          }
        }
      }
      ...
      // Method 2: Direct XPath approach
      try {
        const ownerCell = document.evaluate(
          "//kat-table-cell[contains(text(),'Owner')]/following-sibling::kat-table-cell[1]",
          document,
          null,
          XPathResult.FIRST_ORDERED_NODE_TYPE,
          null
        ...
        if (ownerCell) {
          return ownerCell.textContent.trim();
        }
      } catch (e) {
        console.log('[ILAC Auto Attach] XPath method for owner failed:', e);
      }

      // Method 3: Find cell containing "Owner" and get next cell
      const allCells = document.querySelectorAll('kat-table-cell');
      for (let i = 0; i < allCells.length; i++) {
        const cellText = allCells[i].textContent.trim();
        if ((cellText === 'Owner' || cellText === 'Case Owner') && allCells[i + 1]) {
          return allCells[i + 1].textContent.trim();
        }
      }
    ...
    function ilacShowNotification(message, type) {
      ...
      console.log('[ILAC Auto Attach] Script started...');

      const loadPage = setInterval(async () => {
        const caseId = unsafeWindow?.caseId;
        const userId = unsafeWindow?.userDetails?.agentLogin;
        const caseStatus = ilacGetCaseStatus();
        const caseOwner = ilacGetCaseOwner(); // Get from page instead of unsafeWindow

        // Debug: Log all values being checked
        console.log('[ILAC Auto Attach] Checking values:', {
          caseId: caseId,
          userId: userId,
          caseOwner: caseOwner,
        ...
        if (caseId && userId && caseOwner && caseStatus) {
          clearInterval(loadPage);

          console.log(`[ILAC Auto Attach] All values loaded - User: "${userId}", Owner: "${caseOwner}", Status: "${caseStatus}"`);

          // Check if current user owns the case
          if (userId !== caseOwner) {
            console.log(`[ILAC Auto Attach] SKIPPED - Case owned by "${caseOwner}", not "${userId}"`);
            return;
          }

          // Check if status is Work-in-Progress
          if (caseStatus !== 'Work-in-Progress') {
            console.log(`[ILAC Auto Attach] SKIPPED - Status is "${caseStatus}", not "Work-in-Progress"`);
            return;
          }

          console.log(`[ILAC Auto Attach] PASSED all checks. Looking for shipment ID...`);
          ...
          try {
            ...
            const findShipmentInterval = setInterval(async () => {
              ...
              console.log(`[ILAC Auto Attach] Attempt ${attempts}: Shipment ID = "${shipmentId}"`);

              if (shipmentId) {
                clearInterval(findShipmentInterval);
                console.log(`[ILAC Auto Attach] Found shipment: ${shipmentId}. Attaching report...`);

                try {
                  const result = await ilacAttachReport(caseId, shipmentId);
                  console.log('[ILAC Auto Attach] API Response:', result);

                  if (result?.results?.data?.success) {
                    console.log(`[ILAC Auto Attach] SUCCESS - Report attached!`);
                    ilacShowNotification("✅ ILAC Report attached successfully!", "success");
                  } else {
                    console.error(`[ILAC Auto Attach] FAILED:`, result?.results?.data?.message || result);
                    ilacShowNotification("❌ Failed to attach ILAC Report", "error");
                  }
                } catch (apiError) {
                  console.error('[ILAC Auto Attach] API Error:', apiError);
                  ilacShowNotification("❌ Error attaching ILAC Report", "error");
                }
              } else if (attempts >= maxAttempts) {
                clearInterval(findShipmentInterval);
                console.log("[ILAC Auto Attach] FAILED - No shipment ID found after 30 attempts");
              }
            }, 500);
